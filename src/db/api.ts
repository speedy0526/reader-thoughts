...
export const articlesApi = {
  ...
  // 创建文章
  async createArticle(article: Omit<Article, 'id' | 'created_at'>): Promise<Article | null> {
    const { data, error } = await supabase
      .from('articles')
      .insert(article)
      .select()
...
export const thoughtsApi = {
  // 创建思考
  async createThought(thought: Omit<Thought, 'id' | 'created_at'>): Promise<Thought | null> {
    const { data, error } = await supabase
      .from('thoughts')
      .insert(thought)
      .select()
